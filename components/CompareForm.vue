...
<script>
// Optionally, prepopulate the data.
...
export default {
  ...
  methods: {
    ...
    // Determine the values for each cell in the row
    columnCompare: class Compare {
      ...
      //   Determine whether the field exists on both objects.
      both_exists() {
      ...
      // Determine whether the value of the field is the same data type in both objects.
      type_match() {
        if (!this.both_exists()) return false;
        return typeof this.oldObject[this.field] === typeof this.newObject[this.field]
      }

      // Determine whether the length of the value is the same in both objects.
      length_match() {
        if (!this.both_exists()) return false;
      ...
      // Determine whether the value is the same in both objects.
      value_match() {
        if (!this.both_exists()) return false;
      ...
      result_class() {
        if (!this.both_exists() || (!this.value_match() && !(typeof this.oldObject[this.field] === string && this.oldObject[this.field] === boolean) )) return "fail";
        if (!this.type_match() || !this.length_match() || (!this.value_match() && (typeof this.oldObject[this.field] === string && this.oldObject[this.field] === boolean) ) ) return "investigate";
        return "pass"
      }
    },

    // onClick fucntion to change the active view in the results.
    setActiveView(view) {
      this.activeView = view;
    },

    // Loop through the fields and compare them. Loops unlimited through nested objects.
    ...
    // onClick function for the CompareForm. 
